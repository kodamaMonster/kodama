...
KODAMA is a four-layer system: blockchain, API, agent, and client. Each layer has a distinct responsibility, and communication flows through well-defined interfaces.
...
### On-chain Program (`programs/kodama/`)
...
- `KodamaClient` -- Main entry point, handles connection and signing
...
- `kodama register <name>` -- Register trainer
- `kodama status` -- View profile
- `kodama hunt` -- Find wild monsters
- `kodama gym <id>` -- Challenge gym
- `kodama heal` -- Heal team
- `kodama move <zone>` -- Move to zone
