...
const API_BASE = "https://api.kodama.com/api/v1";
...
function loadConfig(): Config {
  const configPath = path.join(os.homedir(), ".kodama", "config.json");
...
program
  .name("kodama")
  .description("KODAMA CLI -- interact with the pixel monster world")
  .version(VERSION);
...
program
  ...
  .action(async (name: string, opts: { keypair?: string }) => {
    ...
    try {
      ...
      console.log(`  Starter monster assigned. Use 'kodama status' to view.`);
    } catch (err: any) {
  ...
  .action(async (opts: { keypair?: string; autoCapture?: boolean }) => {
    ...
    try {
      ...
      if (opts.autoCapture) {
        ...
        console.log(
          `\n  Use 'kodama battle ${result.battleId}' to continue.`
        );
      }
    } catch (err: any) {
