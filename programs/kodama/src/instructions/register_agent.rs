...
pub fn handler(
    ...
    require!(name.len() >= 1, KodamaError::AgentNameEmpty);
    require!(name.len() <= MAX_NAME_LEN, KodamaError::AgentNameTooLong);
    require!(strategy <= 4, KodamaError::InvalidStrategy);

    // Validate starter species (must be one of the three starters)
    require!(
        starter_species == 1 || starter_species == 4 || starter_species == 7,
        KodamaError::InvalidSpeciesId
    );

    let species = get_species(starter_species).ok_or(KodamaError::InvalidSpeciesId)?;
    let clock = Clock::get()?;
