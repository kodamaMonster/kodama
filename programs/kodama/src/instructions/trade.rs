...
pub struct TradeMonsters<'info> {
    ...
    #[account(
        ...
        bump = agent_a.bump,
        constraint = agent_a.owner == owner_a.key() @ KodamaError::MonsterOwnerMismatch,
    )]
    ...
    #[account(
        ...
        bump = agent_b.bump,
        constraint = agent_b.owner == owner_b.key() @ KodamaError::TradePartnerMismatch,
    )]
    pub agent_b: Account<'info, AgentAccount>,

    #[account(
        mut,
        constraint = monster_a.owner == agent_a.key() @ KodamaError::MonsterOwnerMismatch,
    )]
    pub monster_a: Account<'info, MonsterAccount>,

    #[account(
        mut,
        constraint = monster_b.owner == agent_b.key() @ KodamaError::MonsterOwnerMismatch,
    )]
...
pub fn handler(ctx: Context<TradeMonsters>) -> Result<()> {
    ...
    require!(
        ...
        KodamaError::CannotTradeWithSelf
    );
