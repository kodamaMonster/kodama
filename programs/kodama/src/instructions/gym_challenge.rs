...
pub struct GymChallenge<'info> {
    ...
    #[account(
        mut,
        constraint = monster.owner == agent.key() @ KodamaError::MonsterOwnerMismatch,
    )]
...
pub fn handler(ctx: Context<GymChallenge>, gym_id: u8) -> Result<()> {
    let gym = get_gym_leader(gym_id).ok_or(KodamaError::InvalidGymId)?;
    let agent = &ctx.accounts.agent;

    // Check prerequisites
    require!(!agent.has_badge(gym_id), KodamaError::GymAlreadyDefeated);
    require!(agent.badge_count() >= gym.badge_required, KodamaError::InsufficientBadges);

    let monster = &ctx.accounts.monster;
    require!(!monster.is_fainted, KodamaError::MonsterFainted);
    require!(monster.current_hp > 0, KodamaError::MonsterFainted);
    require!(
        agent.find_in_team(monster.key()).is_some(),
        KodamaError::MonsterNotInTeam
    );
    ...
    let gym_species = get_species(gym.monster_species).ok_or(KodamaError::InvalidSpeciesId)?;
