...
pub fn handler(ctx: Context<EvolveMonster>) -> Result<()> {
    ...
    require!(monster.evolution_id > 0, KodamaError::NoEvolutionAvailable);
    require!(monster.evolution_level > 0, KodamaError::NoEvolutionAvailable);
    require!(monster.level >= monster.evolution_level, KodamaError::EvolutionLevelNotMet);
    require!(
        agent.find_in_team(monster.key()).is_some(),
        KodamaError::MonsterNotInTeam
    );

    let evolved_species = get_species(monster.evolution_id).ok_or(KodamaError::InvalidSpeciesId)?;
