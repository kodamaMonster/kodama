...
export interface KodamaClientConfig {
...
/**
 * Main client for interacting with the KODAMA on-chain program.
 */
export class KodamaClient {
  ...
  constructor(config: KodamaClientConfig) {
    this.connection = config.connection;
    this.wallet = config.wallet;
    this.programId = config.programId ?? PROGRAM_ID;
    this.commitment = config.commitment ?? "confirmed";
    this.logger = config.logger ?? createSilentLogger("KodamaClient");
  }

  /**
   * Create a KodamaClient connected to a given RPC endpoint.
   */
  static connect(
    rpcUrl: string,
    wallet: Keypair,
