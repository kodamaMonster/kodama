...
import { KodamaClient } from "../src/client";
import { PROGRAM_ID, SEEDS } from "../src/constants";

describe("KodamaClient", () => {
  let client: KodamaClient;
  let wallet: Keypair;

  beforeEach(() => {
    wallet = Keypair.generate();
    client = new KodamaClient({
  ...
  describe("constructor", () => {
    ...
    it("should create client with custom program ID", () => {
      ...
      const customClient = new KodamaClient({
        connection: new Connection("http://localhost:8899"),
        wallet,
        programId: customId,
      });
  ...
  describe("connect static method", () => {
    it("should create a connected client", () => {
      const c = KodamaClient.connect("http://localhost:8899", wallet);
      expect(c).toBeInstanceOf(KodamaClient);
  ...
  describe("ping", () => {
    it("should return false for unreachable endpoint", async () => {
      const badClient = KodamaClient.connect(
        "http://localhost:1",
        wallet
      );
